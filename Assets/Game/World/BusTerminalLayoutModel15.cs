...
namespace Bussigo.Game.World
{
    ...
    public class BusTerminalLayoutModel15
    {
        public string TerminalCode => "TERM-SOUTH-15";
        public string TerminalNameEnglish => "Major South Bus Station Hub 15";
        public string TerminalNameTelugu => "ప్రధాన బస్ స్టేషన్ కాంప్లెక్స్ 15";
        public int TotalPlatformBays { get; set; } = 48;
        public List<BusPlatformBay> Platforms { get; } = new List<BusPlatformBay>();

        public BusTerminalLayoutModel15()
        {
            for (int b = 1; b <= TotalPlatformBays; b++)
            {
                ...
                {
                    BayNumber = b,
                    DestinationSignboardEnglish = $"Platform 15-{b:D2} Express",
                    DestinationSignboardTelugu = $"ప్లాట్‌ఫారం 15-{b:D2} ఎక్స్‌ప్రెస్",
