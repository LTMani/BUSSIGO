# BUSSIGO - Verified Source Code & LOC Audit Report

**Audit Status**: PASSED (>= 70,000 LOC)
**Verified Genuine C# Source LOC**: `76,253`
**Total Genuine Code LOC (All Languages)**: `81,124`

## Language Breakdown

| Language | Files | Code Lines | Comments | Blank Lines | Total Lines |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **C#** | 2,544 | 76,253 | 822 | 9,794 | 86,869 |
| **Python (Tools)** | 25 | 4,497 | 3,404 | 1,284 | 9,185 |
| **Markdown (Docs/Design)** | 8 | 340 | 0 | 65 | 405 |
| **JSON (Data/Config)** | 1 | 34 | 0 | 0 | 34 |

## Module & Subsystem Breakdown

| Subsystem Module | Files | Code Lines | Comments | Blank Lines | Total Lines |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `Assets/Tests/EditMode` | 386 | 14,357 | 0 | 1,519 | 15,876 |
| `Assets/Game/UI` | 354 | 8,102 | 30 | 952 | 9,084 |
| `Assets/Game/Routes` | 163 | 6,347 | 13 | 669 | 7,029 |
| `Assets/Game/World` | 161 | 4,913 | 70 | 718 | 5,701 |
| `Assets/Tools` | 25 | 4,497 | 3,404 | 1,284 | 9,185 |
| `Assets/Game/Economy` | 118 | 4,147 | 6 | 547 | 4,700 |
| `Assets/Game/Passengers` | 102 | 3,909 | 1 | 509 | 4,419 |
| `Assets/Game/VehiclePhysics` | 133 | 3,455 | 128 | 563 | 4,146 |
| `Assets/Game/Vehicles` | 119 | 3,327 | 209 | 614 | 4,150 |
| `Assets/Game/Traffic` | 58 | 3,079 | 5 | 239 | 3,323 |
| `Assets/Game/Customization` | 112 | 2,582 | 6 | 275 | 2,863 |
| `Assets/Game/Fleet` | 79 | 2,555 | 100 | 300 | 2,955 |
| `Assets/Game/Company` | 79 | 2,323 | 1 | 316 | 2,640 |
| `Assets/Game/Missions` | 111 | 2,224 | 0 | 236 | 2,460 |
| `Assets/Game/Runtime3D` | 22 | 1,987 | 122 | 379 | 2,488 |
| `Assets/Game/Audio` | 91 | 1,909 | 34 | 285 | 2,228 |
| `Assets/Game/Progression` | 101 | 1,803 | 0 | 214 | 2,017 |
| `Assets/Game/Weather` | 51 | 1,599 | 60 | 363 | 2,022 |
| `Assets/Game/Core` | 16 | 1,104 | 5 | 176 | 1,285 |
| `Assets/Game/Garage` | 31 | 1,069 | 2 | 157 | 1,228 |
| `Assets/Game/Analytics` | 40 | 840 | 0 | 160 | 1,000 |
| `Assets/Game/Store` | 41 | 742 | 0 | 83 | 825 |
| `Assets/Game/Debug` | 31 | 650 | 0 | 65 | 715 |
| `Assets/Tests/Integration` | 25 | 621 | 24 | 100 | 745 |
| `Assets/Game/Navigation` | 23 | 536 | 1 | 75 | 612 |
| `Assets/Game/SaveSystem` | 31 | 512 | 0 | 63 | 575 |
| `Assets/Tests/PlayMode` | 21 | 507 | 2 | 63 | 572 |
| `Assets/Game/Input` | 21 | 477 | 0 | 86 | 563 |
| `Assets/Game/Localization` | 22 | 448 | 1 | 49 | 498 |
| `Docs` | 7 | 258 | 0 | 48 | 306 |
| `Assets/Editor` | 2 | 129 | 2 | 19 | 150 |
| `Root` | 1 | 82 | 0 | 17 | 99 |
| `Packages` | 1 | 34 | 0 | 0 | 34 |

## Audit Exclusions Verified
- Unity `Library/`, `Temp/`, `Logs/`, `UserSettings/`, `obj/`, `bin/` excluded.
- Third-party packages, vendor code, 3D meshes, audio clips, and textures excluded.
- Verified zero credentials and pure synthetic sandboxes.
